...
def exp_rough_vol(N_MC,N_obs,hurst=0.4,N_bags=100, N_items=15, spec_param={'alpha':[2,9],'m':[0.000025,0.000025],'nu':[1.,1.],'mu':[0.0003,0.0003],'Y0_1':[600,600],'Y0_2':[0.001,0.001]},device=torch.device('cuda')):
    ...
    for i in range(N_MC):
        ...
        for j,param in enumerate(params):
            ...
            input_ = np.concatenate([np.array(np.array(input_)[:, :, :, k]) for k in range(N)], axis=2)
            train_RBF_rmse, train_RBF_r2, test_RBF_rmse, test_RBF_r2 = experiments.naive_experiment(input_, y_scaled, train_indices, test_indices,ARD=False,
                                                               param_init=[10, 0, 0,10],device=device)
    ...
    df = pd.concat([df_train_RBF_rmse,df_train_RBF_r2,df_test_RBF_rmse,df_test_RBF_r2,df_train_Sig_rmse,df_train_Sig_r2,df_test_Sig_rmse,df_test_Sig_r2,df_train_pathwise_Sig_rmse,df_train_pathwise_Sig_r2,df_test_pathwise_Sig_rmse,df_test_pathwise_Sig_r2])
    pickle.dump(df,open( "exp_rough_vol.obj", "wb" ))

def exp3(N_MC, subsampling, N_bags=100, N_items=30, tspan=np.linspace(0, 1, 200),
         spec_param={'theta_1': [0, 2], 'Y0': [0., 0.]}, device=torch.device('cuda')):
    ...
    for i in range(N_MC):
        ...
        for j, param in enumerate(params):
            ...
            ''' PREPARE DATA FOR REGRESSION '''
            data_scaled, y_scaled, train_indices, test_indices = utils.split_standardize(example.labels,
                                                                                         example.paths,
                                                                                         standardized=False,
                                                                                         method='stratify')

            X_aug = utils.add_dimension(data_scaled, add_time=False, lead_lag=0)
            X_aug_sub = utils.add_dimension(data_scaled[:, :, ::param, :], add_time=False, lead_lag=0)
            ...
            # Train and Predict
            train_Sig_rmse, train_Sig_r2, test_Sig_rmse, test_Sig_r2 = experiments.experiment_precomputed(
                K_precomputed, y_scaled, train_indices, test_indices, RBF=False,
                plot=False, device=device)
            ...
            # pathwise expected sig
            expected_pathwise_sig = signature_features.scaled_pathwise_expected_iisignature(
                [e.copy() for e in X_aug],
                sig_level1)
            ...
            # Train and Predict
            train_pathwise_Sig_rmse, train_pathwise_Sig_r2, test_pathwise_Sig_rmse, test_pathwise_Sig_r2 = experiments.experiment_precomputed(
                K_precomputed, y_scaled, train_indices, test_indices,
                plot=False)
    ...
    df = pd.concat(
        [df_train_RBF_rmse, df_train_RBF_r2, df_test_RBF_rmse, df_test_RBF_r2, df_train_Sig_rmse, df_train_Sig_r2,
         df_test_Sig_rmse, df_test_Sig_r2, df_train_pathwise_Sig_rmse, df_train_pathwise_Sig_r2,
         df_test_pathwise_Sig_rmse, df_test_pathwise_Sig_r2])
