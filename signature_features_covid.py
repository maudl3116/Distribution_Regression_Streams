...
def scaled_expected_sig(X,level_sig,M,a, ilya_rescale, return_norms=False,weights=None):
    expected_sig = []
    maxi = M*(1.+1./a)
    print('maxi',maxi)
    ...
    for bag in range(len(X)):
        ...
        if weights is None:
            expected_sig.append(np.mean(np.array(bag_sig),axis=0))
        else:
            #print(np.array(weights[bag]))
            expected_sig.append(np.average(np.array(bag_sig),axis=0,weights=np.array(weights[bag])))
    if return_norms:
        return np.array(expected_sig), bags_before, bags_after
    else:
        return np.array(expected_sig)
