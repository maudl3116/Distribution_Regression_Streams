...
def naive_experiment_arbitrary(x, y, train_index,ARD=False,RBF_top=False,param_init=[0,0,0],plot=False,device=torch.device("cuda")):
    ...
    model = GP_naive_arbitrary_items.GP(x_train, torch.tensor(y_train, dtype=torch.float64), param_init[0], param_init[1],param_init[2], param_init[3],
...
def experiment_precomputed(K_precomputed, y, train_index, test_index, param_init=[0,0,0],RBF=False,plot=False,device=torch.device("cpu"),ARD_full=False):
...
def experiment_ARD(data, y, d,level_sig, train_index, test_index, param_init=[0,0,0],RBF=False,plot=False,device=torch.device("cpu"),full=False):
    y_train, y_test = y[train_index], y[test_index]
    ...
    if RBF:
        if full:
            model = GP_sig_ARD_full.GP(x_train_torch, torch.tensor(y_train, dtype=torch.float64, device=device),param_init[0], param_init[1], param_init[2],
