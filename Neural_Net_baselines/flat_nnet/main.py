...
parser.add_argument('--reg', type=float, default=10e-5, metavar='R',
                    help='weight decay')
parser.add_argument('--batch_size', type=int, default=10, metavar='R',
                    help='batch size')
...
def train(epoch,ax=None):
    ...
    for batch_idx, (data, bag_label) in enumerate(train_loader):
        ...
        # calculate loss
        y_pred = model(data)
        loss = criterion(y_pred,bag_label)

        preds_plot.append(y_pred.detach().cpu().numpy())
        labels_plot.append(bag_label.detach().cpu().numpy())

        train_loss += loss.data*data.shape[0]

        # backward pass
        loss.backward()
        optimizer.step()

        nb_bags+=data.shape[0]
    ...
    if not ax is None:
        ...
        plot_fit(ax, np.array(labels_plot).flatten(),np.array(preds_plot).flatten())
...
def test(ax):
    ...
    for batch_idx, (data,bag_label) in enumerate(test_loader):
        ...
        loss = criterion(y_pred[0],bag_label)
        labels.append(bag_label.detach().cpu().numpy())
        test_loss += loss.data
    ...
    plot_fit(ax, np.array(labels).flatten(), np.array(predictions).flatten())
