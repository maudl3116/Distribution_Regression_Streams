...
class NdviDataset(Dataset):
    ...
    def __init__(self, data_file,label_file,train=True,nb_train=200):
        ...
        if self.train:
            self.train_bags_list, self.train_labels_list = [[e for e in self.ndvi_data[i][:10]] for i in range(nb_train)], [self.labels[i][0] for i in range(nb_train)]
        else:
            self.test_bags_list, self.test_labels_list = [[e for e in self.ndvi_data[i][:10]] for i in range(nb_train,len(self.labels))], [self.labels[i][0] for i in range(nb_train,len(self.labels))]
