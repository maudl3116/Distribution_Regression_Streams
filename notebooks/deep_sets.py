...
#         self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(self.optim, factor=0.5, patience=50, verbose=True)
    ...
    def fit(self, train):
        ...
        for j in tqdm(range(self.num_epochs)):
            ...
            if j%50==0:
                print('Train loss: {0:.4f}'.format(loss.data.cpu().numpy()))
    ...
    def evaluate(self, test):
        ...
        for X, y in test_iterator:
            ...
            sum_mse += self.l(y_pred, Variable(y)).data.cpu().numpy()
...
class DeepSetRNN(nn.Module):
    ...
    def init_hidden(self, batch_size):
        return torch.zeros(1, batch_size, self.out_features)
...
class DeepSetLSTM(nn.Module):
    ...
    def init_hidden(self, batch_size):
        return torch.zeros(1, batch_size, self.out_features), torch.zeros(1, batch_size, self.out_features)
