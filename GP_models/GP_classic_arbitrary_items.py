...
class GP():
    ...
    def __init__(self, X, Y, l_init, var_init, noise_init, l_init_top=None,
                 param_list=['lengthscale', 'variance', 'noise'], ARD=False, dtype=torch.float64,
        ...
        if 'lengthscale' in param_list:
            if ARD:
                self.lengthscale = torch.nn.Parameter(
                    l_init * torch.ones(X[0].shape[0], dtype=self.dtype, device=self.device))
            else:
                self.lengthscale = torch.nn.Parameter(l_init * torch.ones(1, dtype=self.dtype, device=self.device))
                self.lengthscale_top = torch.nn.Parameter(
                    l_init_top * torch.ones(1, dtype=self.dtype, device=self.device))
    ...
    def K_eval(self, x):
        ...
        dist = -2 * torch.tensordot(x_, y, [[-2], [-2]]).transpose(1, 2).reshape(x.shape[0], y.shape[0],
                                                                                 self.n_items ** 2)
        dist_mask = -2 * torch.tensordot(self.mask, self.mask, [[-2], [-2]]).transpose(1, 2).reshape(x.shape[0],
                                                                                                     y.shape[0],
                                                                                                     self.n_items ** 2)
    ...
    def obj(self):
        ...
        Lk = torch.triangular_solve(
            self.Y - self.mean_constant * torch.ones_like(self.Y, dtype=self.dtype, device=self.device), L,
            upper=False)[0]
    ...
    def predict(self, X_train, X_test, RBF_top=False, test=False):
