...
def exp1(N_MC,subsample,N_bags=100, N_items=30, nb_periods=20,spec_param={'C':[1.,1.],'phi':[np.pi/8,np.pi/2],'em':[1,1],'omega':[5,5]}):
    ...
    df_train_pathwise_Sig_r2 = pd.DataFrame(index=N_MC * ['train_pathwise_Sig_r2'], columns=subsample)
    df_train_pathwise_Sig_rmse = pd.DataFrame(index=N_MC * ['train_pathwise_Sig_rmse'], columns=subsample)
    df_test_pathwise_Sig_r2 = pd.DataFrame(index=N_MC * ['test_pathwise_Sig_r2'], columns=subsample)
    df_test_pathwise_Sig_rmse = pd.DataFrame(index=N_MC * ['test_pathwise_Sig_rmse'], columns=subsample)
    ...
    for i in range(N_MC):
        for j,param in enumerate(subsample):
            ...
            # pathwise expected sig
            expected_pathwise_sig = signature_features.scaled_pathwise_expected_iisignature([e.copy() for e in data_scaled],
                                                                                            sig_level1,M=1000,
                                                                  a=1, ilya_rescale=True, return_norms=False)
    ...
    pickle.dump(df,open( "exp_phase_circuit_pathwise.obj", "wb" ))
