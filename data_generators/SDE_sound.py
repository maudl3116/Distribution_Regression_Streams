...
class sde():
    ...
    def __init__(self,N_bags=100, N_items=15, t_span=np.linspace(0,1,500),spec_param={'lambda':[1,3]}):
        self.path = None
        self.labels = None
    ...
    def generate_data(self):
        ...
        lambda_ = (self.spec_param['lambda'][1] - self.spec_param['lambda'][0]) * np.random.rand(self.N_bags) + self.spec_param['lambda'][0]*np.ones(self.N_bags)
        ...
        for t in range(1,len(self.t_span)):
            paths[t,:,:] = paths[t-1,:,:] + (self.r+self.R)*self.increments[t-1,:,:,1] + self.lambda_*
