...
class Area():
    ...
    def __init__(self, N_bags=100, N_items=15, t_span=np.linspace(0, 2 * np.pi, 100), spec_param={'a': [1., 2.], 'b': [1., 2.]}):
        self.N_bags = N_bags
    ...
    def generate_data_GBM(self):
        ...
        BM_samples = utils.brownian(steps=len(self.t_span) - 1, width=2 * self.N_bags * self.N_items,
                                    time=self.t_span[-1])
        # self.samples = BM_samples.reshape(len(self.t_span),self.N_bags,self.N_items,2)
        self.samples = BM_samples.reshape(len(self.t_span), self.N_bags, self.N_items, 2)


        paths = b[None,:,None,None]*self.samples  + np.exp((a[None,:,None,None]-0.5*b[None,:,None,None]**2)*self.t_span[:,None,None,None])
