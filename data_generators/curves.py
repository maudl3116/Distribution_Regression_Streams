...
class Helix():
    ...
    def generate_data(self, N_bags=100, N_items=15, t_span=np.linspace(0, 2 * np.pi, 100),
                      spec_param={'a': [1., 2.], 'b': [1., 100.]}, stdv_pos=0, noise='gaussian', stdv_noise=0):
        ...
        b = (spec_param['b'][1] - spec_param['b'][0]) * np.random.rand(N_bags,N_items) + spec_param['b'][0] * np.ones((N_bags,N_items))

        if stdv_pos != 0:
            pos_x = stdv_pos * np.random.randn(N_bags)
            pos_y = stdv_pos * np.random.randn(N_bags)
            pos_z = stdv_pos * np.random.randn(N_bags)
        else:
            pos_x = np.zeros(N_bags)
            pos_y = np.zeros(N_bags)
            pos_z = np.zeros(N_bags)
    ...
    def get_e3(self):
        # second excentricity
        self.e3 = np.sqrt(np.maximum(self.b, self.a) ** 2 - np.minimum(self.b, self.a) ** 2) / np.sqrt(
            np.maximum(self.b, self.a) ** 2 + np.minimum(self.b, self.a) ** 2)
        self.labels = self.e3
